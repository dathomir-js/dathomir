...
#feature_spec(
  name: "append / insert",
  summary: [
    `append` は親ノードの末尾に子ノードを追加する。`insert` はアンカーノードの前に子ノードを挿入し、アンカータイプに応じた SSR/CSR クリーンアップを行う。
  ],
  api: [
    ```typescript
    ...
    - `Iterable`: 配列や iterable に含まれる child を順に展開して挿入
    ...
    *初回呼び出し（SSR アンカーの場合）*:
    ...
    + アンカーが `dh:` プレフィックスのコメントノードかを確認
    + `anchor.nextSibling` から次の `<!--dh:...-->` マーカーまでのノードを収集して削除
    + 新しいコンテンツを `anchor` の前に挿入
    + 挿入したノードを `WeakMap` で追跡
    ...
    *2回目以降の呼び出し（リアクティブ更新）*:
    ...
    + `WeakMap` から前回挿入したノードリストを取得
    + すべてのノードを DOM から削除
    + 新しいコンテンツを `anchor` の前に挿入
    + 挿入したノードを `WeakMap` で追跡（上書き）
  ],
  test_cases: [
    *append*:
    - 子要素の追加
    ...
    *insert*:
    - アンカー前への挿入
    - アンカーが null の場合の末尾追加
    - DocumentFragment の挿入
    - ファクトリ関数からの挿入
    - DocumentFragment を含む iterable の挿入
    - Node とテキスト混在の iterable の挿入
    - 再挿入時の前コンテンツクリーンアップ
    - SSR アンカー後の SSR コンテンツ削除
    - CSR アンカーでの兄弟テンプレートノードの保持
    - 不明な型のテキストノード変換
    - SSR コンテンツの初回クリーンアップ後の WeakMap 追跡
    - parent が null の場合の no-op
  ],
  impl_notes: [
...
#adr(
  ...
  [
    ...
    - `Iterable`: 各 child を順番に展開し、`DocumentFragment` / `Node` / text として挿入。`null` / `undefined` / boolean は JSX と同様に無視する
    - その他: `String(value)` でテキストノードに変換（DEV モードで `console.error` を出力。`warn` ではなく `error` — 意図しない型の使用はバグを示すため）
  ],
  [
