...
4. 一旦作業が完了したら、`question` ツールによる組み込み質問 UI を**必ず**使って `作業が完了してますか？` と確認すること。plain text で代用してはいけない。また、同じ確認文を通常の本文で繰り返してはいけない。この確認は、ユーザーの要望が実現しているかを確かめる目的で行う。選択肢は `yes`、`no` にし、自由記述は `question` ツールの自由入力を使って許可すること。回答が `no` または自由記述で作業続行になった場合も、次の完了時に同じ形式で再度確認すること
